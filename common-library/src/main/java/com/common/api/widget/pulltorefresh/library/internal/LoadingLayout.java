...
import com.common.api.widget.pulltorefresh.library.ILoadingLayout;
...
public abstract class LoadingLayout extends FrameLayout implements ILoadingLayout {
	...
	/**修饰动画旋转效果，定义动画变化率，可以使存在的动画效果accelerated(加速)，decelerated(减速),repeated(重复),bounced(弹跳)*/
	...
	private final ImageView mHeaderTipIv;
	...
	private final ImageView mTextHintIv;
	...
	public LoadingLayout(Context context, final Mode mode, final Orientation scrollDirection, TypedArray attrs) {
		...
		if (attrs.hasValue(R.styleable.PullToRefresh_ptrOrientationMode)) {
			orientation = Orientation.mapIntToValue(attrs.getInteger(
					R.styleable.PullToRefresh_ptrOrientationMode, 1));
		}
		if (attrs.hasValue(R.styleable.PullToRefresh_ptrRefreshText)) {
			mRefreshText = attrs.getBoolean(
					R.styleable.PullToRefresh_ptrRefreshText, true);
		}
...
//				break;
		...
		mHeaderTipIv = (ImageView) mInnerLayout.findViewById(R.id.iv_header);
		mHeaderText = (TextView) mInnerLayout.findViewById(R.id.pull_to_refresh_text);
		mHeaderProgress = (ProgressBar) mInnerLayout.findViewById(R.id.pull_to_refresh_progress);
		mSubHeaderText = (TextView) mInnerLayout.findViewById(R.id.pull_to_refresh_sub_text);
		mHeaderImage = (ImageView) mInnerLayout.findViewById(R.id.pull_to_refresh_image);
		mContentLayout = (RelativeLayout) mInnerLayout.findViewById(R.id.rl_content);
		mTextHintIv = (ImageView) mInnerLayout.findViewById(R.id.iv_text_hint);
		...
		switch (mode) {
			...
			default:
				...
				if(mRefreshText) {
					...
					mPullLabel = context.getString(R.string.savor_loading_hint);
					mRefreshingLabel = context.getString(R.string.savor_loading_hint);
					mReleaseLabel = context.getString(R.string.savor_loading_hint);
				}
				break;
		}
